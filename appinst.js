...
function addPWA(aurl,aatitle,imgurl,ico_appid,svg) {
  if (ico_appid) {
    var toaddpre = "new WIN('(link)[" + aurl + "]', '(icn)[" + imgurl + "]', '(title)[" + aatitle + "]', '(os)[false]', '(full)[false]', '(appname)[" + ico_appid + "]');";
  } else {
    var toaddpre = "new WIN('(link)[" + aurl + "]', '(icn)[" + imgurl + "]', '(title)[" + aatitle + "]', '(os)[false]', '(full)[false]', '(appname)[" + aatitle + "]');";    
  }
  if (svg) {
    var toadd = localStorage.getItem("capps") + '<div class="deskapp" title="' + aatitle + '" onclick="' + toaddpre + '" xmlns="http://www.w3.org/2000/svg">' + svg + '</div>'
 } else {
